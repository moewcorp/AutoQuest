...
namespace AutoQuest
{
    internal unsafe class SkipManager : IDisposable
    {
        ...
        public SkipManager()
        {
            //48 89 5C 24 ?? 48 89 74 24 ?? 57 48 ?? ?? ?? 8B ?? ?? ?? ?? ?? 41 ?? ?? ?? 0F ?? ?? ?? ?? ?? ?? ?? 48 7.0
            ...
            OnEventAction = Svc.Hook.HookFromAddress(Svc.SigScanner.ScanText("48 89 5C 24 ?? 48 89 74 24 ?? 57 48 ?? ?? ?? 8B ?? ?? ?? ?? ?? 41 ?? ?? ?? 0F ?? ?? ?? ?? ?? ?? ?? 48"), new EventActionDelegate(EventActionDetour));
            OnEventAction.Enable();
            //40 ?? 41 ?? 41 ?? 41 ?? 48 ?? ?? ?? 48 83 B9
            ...
            OnEventFinish = Svc.Hook.HookFromAddress(Svc.SigScanner.ScanText("40 ?? 41 ?? 41 ?? 41 ?? 48 ?? ?? ?? 48 83 B9"), new EventFinishDelegate(OnEventFinishDetour));
